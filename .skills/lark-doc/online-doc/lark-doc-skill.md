...
- 用户给了明确字数要求（写 N 字 / x-y 字 / x 字左右 / 上下浮动）→ 生成或改写后用 `scripts/count_chars.py`（online-doc skill 根的 `scripts/` 下，对齐飞书「总字数」）校验，按 create/update workflow 的「字数校验」闭环处理（最多 2 轮，不达标如实告知）
