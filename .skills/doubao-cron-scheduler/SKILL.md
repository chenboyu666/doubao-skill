...
- 多轮创建定时任务：用户跨多轮对话逐步补充需求时，输出**多轮总结后的完整需求**，不要丢失任何的细节
- 单轮/多轮更新定时任务：
...
- 在 `query` 中写明：若登录态失效或权限不足，应在最终回复中说明失败原因和需要用户重新授权/登录。
- 对内部文档、日历、邮件、工单、网页监控等权限敏感任务，写清需要访问的具体来源和最小必要范围。

### 需要登录时的引导

- **非常重要** 如果用户创建的定时任务需要用到浏览器，特别是需要登录网站进行操作的，应该在调用好创建定时任务后，直接用云端浏览器工具打开浏览器到对应的网站，让用户接管登录。登录完成后保存的登录态将用于后续该定时任务的云端执行，接管完成后，整个创建定时任务的过程才算真正结束。
...
| `query` | 写任务触发时真正要执行的事，应在这里描述该任务的具体目标、要求、上下文和输出期望。 |
...
**query 字段必须包含的无人值守约束**：
- 写入“本次请求是由「{title}」定时任务到时触发的”，其中 `{title}` 必须与同次调用的 `title` 完全一致。
- 应在 query 中明确说明：定时任务的创建和执行，不可以委派给OrganizeAgent，否则可能丢失登录态
- 还应在 query 中明确说明：这是定时任务自动触发产生的请求，用户触发时不一定在线，因此执行过程中不要调用 askhuman 或任何需要向用户追问/确认的工具；应基于已有信息和可用工具独立完成任务，如确实缺少必要信息则在最终回复中说明无法完成的原因。对于复杂或执行类任务，还应在 query 中要求 AI 在完成整个任务的回答最后再次告诉用户“这是由「{title}」定时任务到时触发的”；如果只是一个简单提醒，则最终回答末尾不需要额外加这句话。  
   - 典型场景包括：
      1. 如果用户要求 AI 定时完成某项任务，应在这里描述该任务的具体目标、要求、上下文和输出期望，并说明这是由对应 title 的定时任务触发的。请注意不要遗漏用户的需求，尤其是当用户多轮描述需求时，要确保记录下每一个要求。
      2. 如果用户希望 AI 定时提醒自己做某事，应把这里写成到点后需要发送给用户的提醒请求，而不是再次描述如何创建定时任务；例如用户说“晚上 7 点提醒我吃饭”，title 可写为“提醒吃饭”，query 可写为“本次请求是由「提醒吃饭」定时任务到时触发的。请告诉用户：「现在应该吃饭了」”。"
...
- 告知已创建的任务名称。
- 告知它会在什么时候提醒或开始执行。
- 复杂任务说明 AI 会怎么做：关键步骤、会检查或处理哪些信息、预期产出。这里必须参考query字段记录下的所有用户要求，不能忽略任何细节。但不要展示无人值守约束。
