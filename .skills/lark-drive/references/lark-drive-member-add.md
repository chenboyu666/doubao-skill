...
| `--member-type` | 是 | member-id 的类型；支持 `email` / `openid` / `unionid` / `openchat` / `opendepartmentid` / `groupid` / `appid` / `wikispaceid`。                               |
...
- **身份支持**：统一以 user 身份执行。
- **部门协作者**：支持 `--member-type=opendepartmentid` 添加部门协作者。
- **通知**：支持 `--need-notification`。
