...
name: lark-base
version: 1.2.2
description: "飞书多维表格（Base）操作：建表、字段、记录、视图、统计、公式/lookup、表单、仪表盘、workflow、角色权限；遇到 Base/多维表格/bitable 或 /base/ 链接时使用。文件导入转 lark-drive。"
metadata:
  requires:
    bins: ["lark-cli"]
...
- 只是认证、初始化配置、处理 scope 或权限授权恢复，与 Base 操作无关。
...
- Base 文档只保留会影响 Base 路径选择的权限规则；认证、初始化、scope 等凭证准备由 agent 平台负责。
...
- 统一以 user 身份操作用户资源（凭证由 agent 平台注入）。
- user 身份报 scope/授权不足，或错误中包含 `permission_violations` / `hint` 时，根据错误响应引导用户解决，不要循环重试。
- `91403` 或明确不可访问错误不要盲目重试。
- `+base-create` / `+base-copy` 后关注返回中的 `permission_grant`，并把用户是否可打开新 Base 告知用户。
...
| `91403` | 无权限访问该 Base，根据错误响应引导用户解决，不要盲目重试 |
