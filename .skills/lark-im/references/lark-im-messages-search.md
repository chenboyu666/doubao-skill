...
> Runs with user identity (`--as user`).
...
For a named group with history/listing intent, resolve the group with `im +chat-search`, then list messages with `im +chat-messages-list --chat-id <chat_id>`.
...
| Permission denied | Search scope not authorized | Have the agent platform grant the current user the `search:message` scope |
